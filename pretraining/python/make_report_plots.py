...
for name in exp_names:
    ...
    dict_dfs_events[name]['epoch'] = step_to_epoch(
            ...
            DICT_BATCH_SIZE_PRETRAIN[get_model_size_from_dir(name)],
            BRWAC_TXT_LEN
            )
...
#plt.show()
# fig.tight_layout()
plt.savefig('../reports/pretraining_subplots.png', dpi = 300)
...
plt.grid()
plt.tight_layout()
plt.savefig('../reports/pretraining_all_sizes.png', dpi = 300)


# Plot with all sizes - with 0.9 smooth factor
fig = plt.figure()
for name,df in dict_dfs_events.items():
    plt.plot(df['epoch'], smooth_array(df['loss'], 0.9),
...
plt.ylabel('Loss')
plt.grid()
plt.savefig('../reports/pretraining_all_sizes_0.9_smooth.png', dpi = 300)
